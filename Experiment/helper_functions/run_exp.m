...
%             end
            ...
            for i = 1:nStimuli
               if strcmp(P.stim_type, 'grate')
                   ...
                   stim(i).ort = round(R.draws{blok}(section, trial, i),0);
                   stim(i).cur_sigma = R.sigma{blok}(section, trial, i); 
               end
            end
...
%             end
            ...
            elseif strcmp(P.stim_type, 'audio')
                beep.truelength(section, trial) = audio(P, scr, stim, beep); % ellipse doesn't need phase info
            end
...
%             if strcmp(P.stim_type, 'grate')   
%             if stim(i).cur_sigma < 0.136 
%                    randomness = binornd(1,0.8);
%                    conf = binornd(4,0.8);
%                elseif stim(i).cur_sigma < 0.068 && stim(i).cur_sigma > 0.136
%                    randomness = binornd(1,0.75);
%                    conf = binornd(4,0.75);
%                elseif stim(i).cur_sigma < 0.034 && stim(i).cur_sigma > 0.068
%                    randomness = binornd(1,0.7);
%                    conf = binornd(4,0.7);
%                elseif stim(i).cur_sigma < 0.017 && stim(i).cur_sigma > 0.034
%                    randomness = binornd(1,0.6);
%                    conf = binornd(4,0.6);
%                elseif stim(i).cur_sigma < 0.0082 && stim(i).cur_sigma > 0.017
%                    randomness = binornd(1,0.55);
%                    conf = binornd(4,0.55);
%                 elseif stim(i).cur_sigma > 0.003 
%                    randomness = binornd(1,0.55);
%                    conf = binornd(4,0.55);
%                end; 
%             end;    
...
%                end;
%                
%             if (strcmp(P.stim_type, 'grate') && strcmp(task_str,'Task A '))
%                 if stim(i).ort <= 0 %&& randonmess == 1
%                     Chat = 1;
%                 else
%                     Chat = 2;
%                 end; 
%             elseif (strcmp(P.stim_type, 'grate') && strcmp(task_str,'Task B '))
%                  if stim(i).ort >= 5 %&& randomness == 1
%                     Chat = 2;
%                  elseif stim(i).ort <= -5 %&& randomness == 1
%                     Chat = 2;
%                  else
%                      Chat = 1;
%                 end; 
%             elseif (strcmp(P.stim_type, 'audio') && strcmp(task_str,'Task A '))
%                 if stim(i).ort <= 2700 %&& randomness == 1 
%                     Chat = 1
%                 else
%                     Chat = 2
%                 end; 
%             elseif  (strcmp(P.stim_type, 'audio') && strcmp(task_str,'Task B '))
%                  if stim(i).ort <= 2485 %&& randomness == 1 
%                     Chat = 2;
%                  elseif stim(i).ort >= 2915 %&& randomness == 1 
%                     Chat = 2;
%                  else
%                      Chat = 1;
%                 end; 
...
% Responses            
            while Chat == 0
                ...
                if strcmp(type, 'Category Training')
                    if keyCode(scr.keyC1) % cat 1
                        Chat = 1;
                    elseif keyCode(scr.keyC2) % cat 2
                        Chat = 2;
                    else
                        continue
                    end
...
%             end
            ...
            if (strcmp(type,'Testing') && two_response)
...
%             end
        ...
        save([subject_name '_' date '_' P.exp_type '_block' num2str(blok) '_section' num2str(section) '.mat']); % save section-by-section
