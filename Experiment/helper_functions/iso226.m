function [amplitudechange] = iso226(frequency, phon)
...
interpolated = spline(freq,spl,xx);
else
 interpolated = spline(f,Tf,xx);
end 

% 85 because headphone calibration sets tones at 85dB
dBdifference = interpolated(frequency)-85;
amplitudechange = 10^(dBdifference/20);


%% amplitudechange = repmat(0,12500,1);
% dBdifference = repmat(0,12500,1);
% for frequency= 1:1:12500
% dBdifference(frequency,1) = interpolated(frequency)-85;
% amplitudechange(frequency,1) = 10^(dBdifference(frequency,1)/20);
% end;
