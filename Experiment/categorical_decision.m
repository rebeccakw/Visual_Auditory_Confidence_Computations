...
switch room_letter
    case 'home'
        screen_width = 47;  % screen width in cm 
        screen_distance = 57; 
        scr.res = [0 0 4096 2304];
        scr.displayHz = 60;

        [scr.key1, scr.key2, scr.key3, scr.key4, scr.key5, scr.key6,...
        scr.key7, scr.key8, scr.key9, scr.key10,scr.keyC1, scr.keyC2] ...
        = deal(30, 31, 32, 33, 34, 37, 38, 39, 45, 46, 7, 13); % This is for keys 1,2,3,4,5,8,9,0,-,=, d(C1),j(C2)
        ...
        dir = '/Users/s4323621/Dropbox/Documents/MATLAB/confidence-master'; % fill me out     
    case 'lab'
        screen_width = 53.5;  % screen width in cm 
        screen_distance = 57; 
        scr.displayHz = 60;

        [scr.key1, scr.key2, scr.key3, scr.key4, scr.key5, scr.key6,...
        scr.key7, scr.key8, scr.key9, scr.key10,scr.keyC1, scr.keyC2] ...
        = deal(49, 50, 51, 52, 53, 56, 57, 48, 189, 187, 70, 74); % This is for keys 1,2,3,4,5,8,9,0,-,=, f,j
        scr.keyenter=13; % return
        scr.keyinsert=8;% normal delete

        scr.fontsize = 42;
        scr.fontstyle = 1;
        f_c_size = 38; % length and width. must be even.
        fw = 1; % line thickness = 2+2*fw pixels
        dir = 'C:\Experiments\Rebecca_West\confidence-master'; % fill me out
end
...
if strcmp(stim_type, 'audio')
    InitializePsychSound(1);

    % Number of channels and Frequency of the sound
    P.nrchannels = 2;
    P.samplingrate = 48000;

    % Length of the beep
    % tone lasts for 300ms with 50ms ramp up and down
    P.beepLengthSecs = 0.4; %total (including ramp up and down)
    P.rampLengthSecs = 0.05; 
    ...
    switch tmp.(fields{f}).category_params.category_type
        case 'same_mean_diff_std'            
            if strcmp(P.stim_type, 'audio')
                tmp.(fields{f}).category_params.sigma_1 = 125;
                tmp.(fields{f}).category_params.sigma_2 = 500;
                tmp.(fields{f}).category_params.mu = 2700;
            else 
                tmp.(fields{f}).category_params.sigma_1 = 3;
                tmp.(fields{f}).category_params.sigma_2 = 12;
                tmp.(fields{f}).category_params.mu = 0;
            end; 
        case 'diff_mean_same_std'
            if strcmp(P.stim_type, 'audio')
                tmp.(fields{f}).category_params.sigma_s = 475; % these params give a level of performance that is about on par withthe original task (above)
                tmp.(fields{f}).category_params.mu_1 = 2300;
                tmp.(fields{f}).category_params.mu_2 = 3100;
            else
...
%     else
%Creates contrast levels
        %Test.category_params.test_sigmas= repmat(0.9,1,8); % on previous rig: exp(-4:.5:-1.5)
        if strcmp(P.stim_type, 'audio')
%             Test.category_params.test_sigmas= round(exp(linspace(1,4,5)),0);
            Test.category_params.test_sigmas= [4, 9, 26, 55];
            Training.category_params.test_sigmas = 70;
        else
%             Test.category_params.test_sigmas= exp(linspace(-4.8,-2,5)); %  previous: exp(linspace(-5.5,-2,6)) 
            Test.category_params.test_sigmas= [0.033, 0.05, 0.067, 0.135];
...
% else
    ...
    Test.n.blocks = 6; % main experiment: 3, feedback experiment: 3
...
%   Test.n.trials = 8*numel(Test.category_params.test_sigmas); % main experiment: 8*, feedback experiment: 6*
    Test.n.trials = 10*numel(Test.category_params.test_sigmas); % main experiment: 8*, feedback experiment: 6*
...
Training.n.blocks = 3; % was 0 before, but 0 is problematic.
...
Test.t.pres = 50;           % time stimulus is on screen. 50 for main, 80 for attention?
%Test.t.pres = 5000; %added by bec while troubleshooting contrast
...
%Added by Bec - for monitor gamma correction     
% PsychImaging('PrepareConfiguration');
% PsychImaging('AddTask', 'FinalFormatting', 'DisplayColorCorrection', 'SimpleGamma'); % allows gamma correction
%added by bec - restore to default settings 
PsychDefaultSetup(0)
...
%Added by Bec 
% [scr.win, scr.rect] = PsychImaging('OpenWindow', screenid, color.bg, [0 0 400 400]);
[scr.win, scr.rect] = PsychImaging('OpenWindow', screenid, color.bg);

%gamma correction 
load ASUS_gammaTable
...
%     
    ...
    P.grateSigma = 1.2; % Gabor Gaussian envelope standard deviation (degrees) 
    P.grateSigma = P.grateSigma * P.pxPerDeg; %...converted to pixels
    P.grateAspectRatio = 1;
    P.grateSpatialFreq = 0.5; % cycles/degree - bec changed from .8 (SHOULD be .5)
...
%     
    ...
    if strcmp(P.stim_type, 'grate')
        ...
        ny = ny+P.grateAlphaMaskSize; % P.grateAlphaMaskSize*2/3?
...
%         ellipse(P, scr, [], stim)
%         ny = ny+max_ellipse_d;
%     elseif strcmp(P.stim_type, 'audio')
%         InitializePsychSound(1);
%         DrawFormattedText(scr.win, 'Example stimulus', 'center', 'center', color.wt);
%         Screen('Flip', scr.win);
%         WaitSecs(1);
%         pahandle = PsychPortAudio('Open', [], 1, 1, 48000, 2);
%         PsychPortAudio('Volume', pahandle, 0.5);
%         myBeep = MakeBeep(1000, 3, 48000);
            ...
            switch task_letter
                case 'A'
                    if strcmp(P.stim_type, 'grate')
                       ...
                       midtxt = 'In Task A, stimuli from Category 1 tend to\n\nbe low-pitched, and stimuli from Category 2\n\ntend to be high-pitched.\n\nSee Task sheet for more info.';
                    end 
                case 'B'
                    if strcmp(P.stim_type, 'grate')
                        ...
                        midtxt = 'In Task B, a medium pitched stimulus is more likely\n\nto be from Category 1, and a strongly high or low\n\npitched stimulus is more likely\n\nto be from Category 2.\n\nSee Task sheet for more info.';
                    end
                otherwise
                    midtxt = '';
            end
...
category_demo
    ...
    %Training
    P.exp_type = 'training';
    for k = 1:Training.n.blocks
     run_exp(Training.n, Training.R, Training.t, scr, ...
   ...
   for k = 1:Test.n.blocks
        run_exp(Test.n, Test.R, Test.t, scr,color, P, 'Testing', k, new_subject, task_str, final_task, subject_name, [], two_response, test_feedback)
    end;
