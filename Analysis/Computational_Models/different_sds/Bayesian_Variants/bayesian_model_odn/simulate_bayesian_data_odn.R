simulate <- function(sub, confidencedata, parameters, modality = 'visual', means = TRUE) {
  ...
  positive_ori_boundaries <- t(apply(cbind(d_boundaries, sigma_list), 1, function(x) d_to_ori(x[1:7], x[8])))
  negative_ori_boundaries <- t(apply(cbind(d_boundaries, sigma_list), 1, function(x) rev(-d_to_ori(x[1:7], x[8]))))
