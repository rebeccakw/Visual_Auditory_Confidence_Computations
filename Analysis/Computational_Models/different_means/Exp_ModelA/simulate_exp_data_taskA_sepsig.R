simulate <- function (sub, parameters) {
  ...
  sigmas <- as.numeric(parameters[9:16]) #sigma parameters
  exp = as.numeric(parameters[17]) #decision noise parameter
  ...
  sigma_list[simulated_data$stim_type == 'Auditory'] = sigmas[4 + simulated_data$stim_reliability_level[simulated_data$stim_type == 'Auditory']]
