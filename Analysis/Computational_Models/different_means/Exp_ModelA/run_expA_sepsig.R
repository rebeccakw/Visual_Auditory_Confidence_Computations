...
source("/Users/rebeccawest/Dropbox/Documents/EndofDay_Confidence_Models/Exp_ModelA/generate_exp_parametersA_sepsig.R")
...
starting_parameters = matrix(starting_parameters[!is.na(starting_parameters)], ncol = ncol(starting_parameters))
...
source("/Users/rebeccawest/Dropbox/Documents/EndofDay_Confidence_Models/Exp_ModelA/exp_model_taskA_sepsig.R")

tic=Sys.time()
output <- foreach(x = 1:nrow(combinations),
                  .combine = "rbind")  %dopar% {
                    output = exp_model_function_sigs(combinations[x, 2], #subject_number
...
source("/Users/rebeccawest/Dropbox/Documents/EndofDay_Confidence_Models/Exp_ModelA/simulate_exp_data_taskA_sepsig.R")
...
simulated_data <- foreach(sub = 1:8,
                          .combine = "rbind")  %dopar% {
                            output = simulate(sub, as.numeric(best_fits[sub, 1:17]))
                          }
toc=Sys.time()
print(toc-tic)
