...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Exp_ModelA/generate_exp_parametersA_odn.R")

confidencedata <- read.csv("/Users/s4323621/Dropbox/Documents/Cluster_Files/visual_taskA_data.csv")
cores = 8
...
n_starting_parameters = 50 #3

tic = Sys.time()
starting_parameters <- foreach(x = 1:n_starting_parameters,
                               .combine = "rbind")  %dopar% {
                                 output = generate_exp_parameters(confidencedata)
                               }
toc = Sys.time()
toc - tic
...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Exp_ModelA/exp_model_taskA_odn.R")

tic=Sys.time()
output <- foreach(x = 1:nrow(combinations),
                  .combine = "rbind")  %dopar% {
                    output = exp_model_function_odn(confidencedata, combinations[x, 2], #subject_number
                                                 100,  #number of repetitions
                                                 starting_parameters[combinations[x, 1],],
                                                 modality = 'visual') #starting_parameters 
                  }
toc=Sys.time()
print(toc-tic)
...
best_fits <- read.csv("/Users/s4323621/Dropbox/Documents/Best_Fitting_Parameters/TaskA/Linear_ODN/visual.csv")

source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Exp_ModelA/simulate_exp_data_taskA_odn.R")
...
simulated_data <- foreach(sub = 1:8,
                          .combine = "rbind")  %dopar% {
                            output = simulate(confidencedata, sub, as.numeric(best_fits[sub, 1:14]))
                          }
toc=Sys.time()
print(toc-tic)
