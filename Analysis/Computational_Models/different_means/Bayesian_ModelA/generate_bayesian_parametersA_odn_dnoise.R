generate_bayesian_parameters_odn_dnoise <- function(modality = 'visual') {
  #============= Parameter Sampling ===========
  nparams = 10
  ...
  #boundary parameters
  boundaries_mu = c(-1.5,	-1,	-6,	-0.2)
  #boundaries_mu = c(-0.2,	-0.6,	-1,	-1.5) #from group model without decision noise 
  ...
  dnoise_mu = 0.5 #from group fits 
  dnoise_sd = 0.5
  dnoise_bounds = c(0, 10)
  ...
  repeat {
    ...
    dnoise <- rnorm(1, dnoise_mu, dnoise_sd)
    ...
    if (all(sigmas == sort(sigmas, decreasing = TRUE)) & #order check
        ...
        (odn > odn_bounds[1]) & 
        (odn < odn_bounds[2]) & 
        (dnoise > dnoise_bounds[1]) & 
        (dnoise < dnoise_bounds[2])) { #upper bound check
      break
    }
  ...
  if (time_out == TRUE) {
    ...
    diff_bounds = diff(c(0, rev(bounds)))
    generating_parameters <- c(diff_bounds, sigmas, odn, dnoise)
  }
