...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/generate_bayesian_parametersA.R")
...
starting_parameters <- foreach(x = 1:n_starting_parameters,
                               .combine = "rbind")  %dopar% {
                                 output = generate_bayesian_parameters('auditory')
                               }
toc = Sys.time()
toc - tic
...
all_starting_parameters = starting_parameters


#if (modality == 'visual') {
#  starting_parameters = cbind(all_starting_parameters, matrix(rep(c(-0.6247561, 0.7809452), each= nrow(starting_parameters)), ncol= 2))
#} 
#if (modality == 'auditory') {
  starting_parameters = cbind(all_starting_parameters, matrix(rep(c(-0.6449836, 0.7608733), each= nrow(starting_parameters)), ncol= 2))
#}

source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/bayesian_model_taskA_freeprior.R")

tic=Sys.time()
output <- foreach(x =1:nrow(combinations),
                  .combine = "rbind")  %dopar% {
                    output = bayesian_model_function_freeprior(combinations[x, 2], #subject_number
...
#                  .combine = "rbind")  %dopar% {
#                    output = bayesian_model_function(1:7, #subject_number
#                                                  100,  #number of repetitions
#                                                 starting_parameters[x,], 
...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/simulate_bayesian_data_taskA_freepriors.R")
...
simulated_data <- foreach(sub = 1:8,
                          .combine = "rbind")  %dopar% {
                            output = simulate(sub, as.numeric(best_fits[sub, 1:10]),
                                              'auditory')
                          }
toc=Sys.time()
print(toc-tic)
...
write.csv(output, "/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/standardised_bayesian_model_auditory_output_.csv")
