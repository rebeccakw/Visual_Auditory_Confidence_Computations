bayesian_model_function_freeprior <- function(sub, reps = 50, starting_parameters, modality = 'visual') {
  ...
  parameter_names <- c("b1", "b2", "b3","b4", 
                       "sigma1", "sigma2", "sigma3","sigma4", "cat_mean", "cat_sigma")
  ...
  for (rep in 1:reps) {   
    ...
    fixed_model <- function(par){
      ...
      b_params = c(-Inf, -measurement(par[1:3], sigma, par[9], par[10]), 
                   measurement(par[4], sigma, par[9], par[10]), 
                   rev(measurement(par[1:3], sigma, par[9], par[10])), Inf)
