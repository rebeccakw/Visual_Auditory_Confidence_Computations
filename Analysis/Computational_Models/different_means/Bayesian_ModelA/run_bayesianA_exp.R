...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/generate_bayesian_parametersA_exp.R")
...
n_starting_parameters =10 #3

tic = Sys.time()
starting_parameters <- foreach(x = 1:n_starting_parameters,
                               .combine = "rbind")  %dopar% {
                                 output = generate_bayesian_parameters_exp('auditory')
                               }
toc = Sys.time()
toc - tic
...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/bayesian_model_taskA_exp.R")

tic=Sys.time()
output <- foreach(x =1:nrow(combinations),
                  .combine = "rbind")  %dopar% {
                    output = bayesian_model_function_exp(combinations[x, 2], #subject_number
...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Bayesian_ModelA/simulate_bayesian_data_taskA_exp.R")
...
simulated_data <- foreach(sub = 1:8,
                          .combine = "rbind")  %dopar% {
                            output = simulate_exp(sub, as.numeric(best_fits[sub, 1:10]),
                                              'auditory')
                          }
toc=Sys.time()
print(toc-tic)
