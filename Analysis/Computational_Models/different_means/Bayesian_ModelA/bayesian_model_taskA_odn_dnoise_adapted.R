bayesian_model_function <- function(sub, reps = 50, starting_parameters, modality = 'visual') {
  ...
  if (modality == 'auditory') {
    ...
    confidencedata$scaled_stim_orientation <- scale(confidencedata$stim_orientation)
  }
  ...
  if (modality == 'auditory') {
    cat_mean = (2300 - mean(confidencedata$stim_orientation))/sd(confidencedata$stim_orientation)
    cat_sigma = (425 - mean(confidencedata$stim_orientation))/sd(confidencedata$stim_orientation)
  }
  ...
  parameter_names <- c("b1", "b2", "b3","b4", 
                       "sigma1", "sigma2", "sigma3","sigma4", "psi", "d_noise")
  ...
  for (rep in 1:reps) {   
    ...
    model <- function(par){
      ...
      if (any(par[5:10] <= 0) | #none of the sigma/dnoise parameters can be less than 0 
      ...
      oris = matrix(rep(confidencedata$scaled_stim_orientation, each = 101), nrow = 101, ncol = length(sigma))
