...
linear_model_function <- function(confidencedata, sub, reps = 50, starting_parameters, modality = 'visual') {
  ...
  for (rep in 1:reps) {   
    ...
    linear_model <- function(par){
      ...
      for (rel in 1:4) {
        ...
        sigma = par[(8 + rel)]
        b_params = c(-Inf, -(par[1:3] + (par[5:7]*sigma)), (par[4] + par[8]*sigma), rev(par[1:3] + (par[5:7]*sigma)), Inf)
