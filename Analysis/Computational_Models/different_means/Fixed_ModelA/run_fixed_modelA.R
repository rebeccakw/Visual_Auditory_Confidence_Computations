...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Fixed_ModelA/generate_fixed_parametersA.R")
...
#successes = length(starting_parameters[starting_parameters != 0])/18
#starting_parameters = matrix(starting_parameters[starting_parameters != 0], successes, 18)
...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Fixed_ModelA/fixed_modelA.R")

tic=Sys.time()
output <- foreach(x = 1:nrow(combinations),
                  .combine = "rbind")  %dopar% {
                    output = fixed_model_function(combinations[x, 2], #subject_number
...
#                  .combine = "rbind")  %dopar% {
#                    output = fixed_model_function(1:7, #subject_number
#                                                  100,  #number of repetitions
...
source("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Fixed_ModelA/simulate_fixed_data_taskA.R")
...
simulated_data <- foreach(sub = 1:8,
                          .combine = "rbind")  %dopar% {
                            output = simulate(sub, as.numeric(best_fits[sub, 1:8]))
                          }
toc=Sys.time()
print(toc-tic)


write.csv(output, paste0("/Users/s4323621/Dropbox/Documents/EndofDay_Confidence_Models/Fixed_Model/standardised_fixed_model_visual_output_", toc, ".csv"))
